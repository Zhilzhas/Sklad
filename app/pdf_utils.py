...
def generate_invoice_pdf(
    ...
    for copy_index in range(1, 4):
        ...
        c.drawString(35, top, f"Накладная № {invoice['invoice_number']} (копия {copy_index}/3)")

        c.setFont(font_regular, 10)
        y = top - 24
        c.drawString(35, y, f"Дата формирования: {_safe_text(invoice['issued_date'], 20)}")
        y -= 15
        c.drawString(35, y, f"Грузоотправитель: {_safe_text(invoice['shipper_name'], 85)}")
        y -= 15
        c.drawString(35, y, f"Телефон отправителя: {_safe_text(invoice['shipper_phone'], 25)}")
        y -= 15
        c.drawString(35, y, f"Грузополучатель: {_safe_text(invoice['consignee_name'], 85)}")
        y -= 15
        c.drawString(35, y, f"Телефон получателя: {_safe_text(invoice['consignee_phone'], 25)}")
